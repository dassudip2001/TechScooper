...
import { getCategoryRecommendations } from "../services/recommendation.service";

export async function recommend(req: Request, res: Response) {
  try {
    const productId = Number(req.params.id);

    const products = await getCategoryRecommendations(productId);
    ...
    // return res.status(201).send({ data: sorted as object[] });
  } catch (err) {
